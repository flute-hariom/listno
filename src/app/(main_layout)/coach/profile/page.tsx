...
import {
  User, Mail, Phone, Award, Star, Clock, Edit, Camera, DollarSign,
  Brain, Heart, Briefcase, BookOpen, Sparkles, Languages, Calendar,
  Check, AlertCircle, Video, MessageSquare, GraduationCap, Shield,
  ChevronRight, TrendingUp, MapPin, Smile, Save, X, ChevronDown, ChevronUp
} from 'lucide-react';
...
export default function CoachProfile() {
  ...
  const defaultData = {
    ...
    profilePhotoPreview: 'https://images.unsplash.com/photo-1679014844834-e86723f36c74?w=400&h=400&fit=crop',
    selectedCategories: ['Mind & Emotions', 'Relationships', 'Career & Work'],
    selectedSubcategories: {
      'Mind & Emotions': ['Stress Management', 'Anxiety Relief', 'Confidence Building', 'Depression Support'],
      'Relationships': ['Breakup Support', 'Marriage Counseling', 'Communication Skills'],
      'Career & Work': ['Work Stress', 'Career Transition', 'Work-Life Balance']
    },
    yearsOfExperience: '8',
    education: 'Ph.D. in Clinical Psychology, Mumbai University | M.A. in Counseling Psychology, Delhi University',
    certifications: [
    ...
    videoPrice: '25',
    availability: {
      monday: { enabled: true, slots: ['9:00 AM - 12:00 PM', '2:00 PM - 6:00 PM', '7:00 PM - 9:00 PM'] },
      tuesday: { enabled: true, slots: ['9:00 AM - 12:00 PM', '2:00 PM - 6:00 PM', '7:00 PM - 9:00 PM'] },
      wednesday: { enabled: true, slots: ['9:00 AM - 12:00 PM', '2:00 PM - 5:00 PM'] },
      thursday: { enabled: true, slots: ['9:00 AM - 12:00 PM', '2:00 PM - 6:00 PM', '7:00 PM - 9:00 PM'] },
      friday: { enabled: true, slots: ['9:00 AM - 12:00 PM', '2:00 PM - 6:00 PM', '7:00 PM - 9:00 PM'] },
      saturday: { enabled: true, slots: ['10:00 AM - 2:00 PM', '4:00 PM - 8:00 PM'] },
  ...
  const categories = [
    { id: 'mind-emotions', name: 'Mind & Emotions', icon: Brain, color: 'from-blue-500 to-cyan-500' },
    { id: 'relationships', name: 'Relationships', icon: Heart, color: 'from-pink-500 to-rose-500' },
    { id: 'career', name: 'Career & Work', icon: Briefcase, color: 'from-purple-500 to-indigo-500' },
    { id: 'education', name: 'Education & Study', icon: BookOpen, color: 'from-amber-500 to-orange-500' },
    { id: 'wellness', name: 'Health & Wellness', icon: Sparkles, color: 'from-green-500 to-emerald-500' },
  ];

  const subcategories = {
    'Mind & Emotions': ['Stress Management', 'Anxiety Relief', 'Confidence Building', 'Depression Support', 'Self-Respect', 'Worry & Anxiety'],
    'Relationships': ['Love & Romance', 'Breakup Support', 'Marriage Counseling', 'Communication Skills', 'Parenting', 'Family Issues'],
    'Career & Work': ['Work Stress', 'Career Transition', 'Work-Life Balance', 'Boss Relationship', 'Career Growth'],
    'Education & Study': ['Exam Stress', 'Study Motivation', 'Focus & Concentration', 'Learning Skills'],
    'Health & Wellness': ['Sleep Issues', 'Fitness Goals', 'Nutrition', 'Self-Care', 'Mindfulness']
  };
  ...
  const stats = [
    { label: 'Total Sessions', value: profileData.totalSessions, icon: Clock, color: 'bg-purple-100 text-purple-600' },
    { label: 'Average Rating', value: profileData.rating, icon: Star, color: 'bg-yellow-100 text-yellow-600' },
    { label: 'Total Earnings', value: `₹${(profileData.totalEarnings / 1000).toFixed(0)}K`, icon: DollarSign, color: 'bg-green-100 text-green-600' },
    { label: 'Total Reviews', value: profileData.totalReviews, icon: Award, color: 'bg-blue-100 text-blue-600' }
  ];
  ...
  const toggleCategory = (categoryName: string) => {
    if (profileData.selectedCategories.includes(categoryName)) {
      setProfileData({
        ...profileData,
        selectedCategories: profileData.selectedCategories.filter(c => c !== categoryName),
        selectedSubcategories: {
  ...
  const toggleSubcategory = (categoryName: string, subcategoryName: string) => {
    const current = profileData.selectedSubcategories[categoryName] || [];
    if (current.includes(subcategoryName)) {
      setProfileData({
        ...profileData,
        selectedSubcategories: {
          ...profileData.selectedSubcategories,
          [categoryName]: current.filter(s => s !== subcategoryName)
        }
      });
    } else {
      setProfileData({
    ...
    setProfileData({
      ...
      availability: {
        ...
        [day]: {
          ...
          enabled: !profileData.availability[day as keyof typeof profileData.availability].enabled
        }
      }
    });
  };
  ...
  const toggleTimeSlot = (day: string, slot: string) => {
    const dayData = profileData.availability[day as keyof typeof profileData.availability];
  ...
  const calculateCompletion = () => {
    ...
    total += 10;
    if (Object.values(profileData.selectedSubcategories).some((subs: any) => subs.length > 0)) completed += 10;
    total += 10;
    ...
    // Pricing (15%)
    if (profileData.chatPrice || profileData.audioPrice || profileData.videoPrice) completed += 15;
    total += 15;

    // Availability (10%)
    const hasAvailability = Object.values(profileData.availability).some((day: any) => day.enabled && day.slots.length > 0);
    if (hasAvailability) completed += 10;
    total += 10;

    // Bank Details (10%)
    if (profileData.accountHolderName && profileData.bankName && profileData.accountNumber && profileData.ifscCode) completed += 10;
    total += 10;
    ...
    <div className="min-h-screen pb-24 bg-gray-50 overflow-x-hidden max-w-[100vw]">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-purple-100 mt-1">Manage your professional information and preferences</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
            ...
            <div className="flex-1 w-full text-center sm:text-left">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <div className="mb-4 sm:mb-0">
                  ...
                  <p className="text-gray-500">Coach since {profileData.joinedDate}</p>
                </div>
              ...
              {isEditing ? (
                ...
                profileData.bio && (
                  <p className="text-gray-600 mb-4 text-sm sm:text-base leading-relaxed">{profileData.bio}</p>
                )
              )}
        ...
        <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-2xl p-6 sm:p-8 shadow-lg mb-6 border-2 border-purple-200">
          ...
          <div className="w-full bg-gray-200 rounded-full h-4 mb-4 overflow-hidden">
            <div 
              className={`h-full rounded-full transition-all duration-500 ${
                ...
                completionPercentage >= 70 ? 'bg-gradient-to-r from-blue-500 to-purple-500' : 
          ...
          {completionPercentage === 100 ? (
            <div className="flex items-center gap-2 text-green-700">
              ...
              <p className="text-sm font-semibold">Your profile is complete! You&apos;re ready to start coaching.</p>
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-gray-700">
                <AlertCircle className="w-5 h-5 text-orange-600" />
                <p className="text-sm font-semibold">Complete your profile to attract more clients</p>
              </div>
              <div className="text-xs text-gray-600 space-y-1 pl-7">
                {!profileData.bio || profileData.bio.length < 50 ? <p>• Add a detailed bio (at least 50 characters)</p> : null}
                {profileData.selectedCategories.length === 0 ? <p>• Select your coaching categories</p> : null}
                {!Object.values(profileData.selectedSubcategories).some((subs: any) => subs.length > 0) ? <p>• Add your specializations</p> : null}
                {!profileData.yearsOfExperience ? <p>• Add your years of experience</p> : null}
                {!profileData.education ? <p>• Add your education details</p> : null}
                {profileData.certifications.length === 0 ? <p>• Add certifications</p> : null}
                {profileData.languages.length === 0 ? <p>• Add languages you speak</p> : null}
                {!profileData.chatPrice && !profileData.audioPrice && !profileData.videoPrice ? <p>• Set your pricing</p> : null}
                {!Object.values(profileData.availability).some((day: any) => day.enabled && day.slots.length > 0) ? <p>• Set your availability</p> : null}
                {(!profileData.accountHolderName || !profileData.bankName || !profileData.accountNumber || !profileData.ifscCode) ? <p>• Add complete bank details</p> : null}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-white rounded-2xl p-4 sm:p-6 shadow-md">
              <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl ${stat.color} flex items-center justify-center mb-3`}>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {isEditing && (
              ...
              >
                {showCategorySelector ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          ...
          {isEditing && showCategorySelector && (
            <div className="mb-4 p-4 border-2 border-purple-200 rounded-xl bg-purple-50">
              <p className="text-sm text-gray-700 mb-3">Select your main categories:</p>
              ...
              {profileData.selectedCategories.map((category) => (
                <div key={category} className="mb-4">
                  <p className="text-sm font-medium text-gray-700 mb-2">{category} - Select specializations:</p>
                    ...
                    {subcategories[category as keyof typeof subcategories]?.map((sub) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                          profileData.selectedSubcategories[category]?.includes(sub)
          ...
          <div className="space-y-4">
            {Object.entries(profileData.selectedSubcategories).map(([category, subcats]) => (
              subcats.length > 0 && (
                <div key={category} className="border-l-4 border-purple-300 pl-4">
                  <h4 className="font-bold text-gray-800 mb-2 flex items-center gap-2">
                    {category === 'Mind & Emotions' && <Brain className="w-5 h-5 text-blue-600" />}
                    {category === 'Relationships' && <Heart className="w-5 h-5 text-pink-600" />}
                    {category === 'Career & Work' && <Briefcase className="w-5 h-5 text-purple-600" />}
                    {category === 'Education & Study' && <BookOpen className="w-5 h-5 text-amber-600" />}
                    {category === 'Health & Wellness' && <Sparkles className="w-5 h-5 text-green-600" />}
                    {category}
                  </h4>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {isEditing && (
              <button
                ...
                className="text-purple-600 font-semibold text-sm flex items-center gap-1"
              >
                {showLanguageSelector ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          ...
          {isEditing && showLanguageSelector && (
            <div className="mb-4 p-4 border-2 border-blue-200 rounded-xl bg-blue-50">
              <p className="text-sm text-gray-700 mb-3">Select languages you speak:</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 border-2 border-blue-200 rounded-xl">
                ...
                <input
                  ...
                  onChange={(e) => setProfileData({ ...profileData, chatPrice: e.target.value })}
              ...
              <div className="p-4 border-2 border-green-200 rounded-xl">
                ...
                <input
                  ...
                  onChange={(e) => setProfileData({ ...profileData, audioPrice: e.target.value })}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-purple-500 focus:outline-none"
                  placeholder="₹/min"
                />
              </div>
              ...
              <div className="p-4 border-2 border-purple-200 rounded-xl">
                ...
                <input
                  ...
                  onChange={(e) => setProfileData({ ...profileData, videoPrice: e.target.value })}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-purple-500 focus:outline-none"
                  placeholder="₹/min"
                />
              </div>
              ...
              {profileData.chatPrice && (
                <div className="p-5 border-2 border-blue-200 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-bold text-gray-800 text-lg">Chat</span>
                  </div>
                  <div className="text-3xl font-bold text-blue-600 mb-1">₹{profileData.chatPrice}</div>
                  <p className="text-sm text-gray-600 font-medium">per minute</p>
                </div>
              )}
              ...
              {profileData.audioPrice && (
                <div className="p-5 border-2 border-green-200 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-bold text-gray-800 text-lg">Voice Call</span>
                  </div>
                  <div className="text-3xl font-bold text-green-600 mb-1">₹{profileData.audioPrice}</div>
                  <p className="text-sm text-gray-600 font-medium">per minute</p>
                </div>
              )}
              ...
              {profileData.videoPrice && (
                <div className="p-5 border-2 border-purple-200 rounded-xl bg-gradient-to-br from-purple-50 to-pink-50">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-bold text-gray-800 text-lg">Video Call</span>
                  </div>
                  <div className="text-3xl font-bold text-purple-600 mb-1">₹{profileData.videoPrice}</div>
                  <p className="text-sm text-gray-600 font-medium">per minute</p>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {isEditing && (
              <button
                ...
                className="text-purple-600 font-semibold text-sm flex items-center gap-1"
              >
                {showAvailabilityEditor ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              ...
              {Object.entries(profileData.availability).map(([day, data]) => (
                <div key={day} className="bg-white p-4 rounded-lg">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    />
                    <label className="font-bold text-gray-800 capitalize">{day}</label>
                  </div>
                  {data.enabled && (
          ...
          {!isEditing && (
            <div className="mt-6 space-y-3 bg-gradient-to-br from-purple-50 to-blue-50 p-4 rounded-xl">
              <h4 className="font-bold text-gray-800 mb-3">Detailed Schedule:</h4>
              {Object.entries(profileData.availability)
                .filter(([_, data]) => data.enabled && data.slots.length > 0)
                .map(([day, data]) => (
                  <div key={day} className="flex items-start gap-3 text-sm bg-white p-3 rounded-lg">
                    <span className="font-bold text-purple-600 capitalize min-w-[100px]">{day}</span>
                    <div className="flex flex-wrap gap-2">
          ...
          {isEditing ? (
            <div className="space-y-4">
              ...
              <div className="mt-4 p-4 bg-green-50 border-2 border-green-200 rounded-xl">
                <p className="text-xs text-green-800 flex items-center gap-2">
                  ...
                  <span className="font-semibold">Your banking details are encrypted and secure. They will only be used for payment processing.</span>
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-5 rounded-xl border-2 border-green-200">
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-800">{profileData.accountHolderName}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-green-200 rounded-full flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-800">{profileData.bankName}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-green-200 rounded-full flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-800">{profileData.accountNumber}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-green-200 rounded-full flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-800">{profileData.ifscCode}</p>
                  </div>
                </div>
                {profileData.upiId && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-gray-800">{profileData.upiId}</p>
                    </div>
                  </div>
                )}
              </div>
              <div className="mt-4 pt-4 border-t border-green-200">
                <p className="text-xs text-green-800 flex items-center gap-2">
                  <Shield className="w-4 h-4" />
                  <span className="font-semibold">Your banking details are encrypted and secure</span>
                </p>
              </div>
            </div>
          )}
